...
?>"
        ...
        case "new_encryption_of_pw":
			...
			// Check SALT length
			if (strlen(SALT) !== 16 && strlen($_SESSION['session_salt']) !== 16) {
				//Generate a random string.
				$token = openssl_random_pseudo_bytes(8);
				 
				// generate key
				$_SESSION['session_salt'] = bin2hex($token);
			}
						
			file_put_contents('update.log', '----'.PHP_EOL . 'Date: '.date('m/d/Y h:i:s a', time()), FILE_APPEND);
			file_put_contents('update.log', PHP_EOL . 'New SALT: '.$_SESSION['session_salt'], FILE_APPEND);
            if ($_POST['suggestion'] != "1") {
                ...
                while ($data = mysqli_fetch_array($res)) {
                    ...
                    if (!empty($data['pw_iv'])) {
						...
						file_put_contents('update.log', PHP_EOL . 'Current password: '.$pw, FILE_APPEND);
						
                        // crypt pw with new protocol #3
                        // encrypt pw
                        $encrypt = cryption($pw, $_SESSION['session_salt'], "", "encrypt");
						
						file_put_contents('update.log', PHP_EOL . 'New encryp: '.print_r($encrypt, true), FILE_APPEND);
